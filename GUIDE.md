...
- **Hardware sync over software timing.** Where the Unit Interface Profile provides a dedicated hardware synchronization signal, the host contract MUST NOT depend on timing derived only from software message latency; drive and document the hardware signal instead. This is a Unit-to-host signal within one Module; the Module-to-Module AURIORA Event Link is a different thing (§14.2).

### 14.2 AURIORA Event Link (AEL)

These rules apply to firmware in a Module that provides an AEL IN or AEL OUT port, and to the firmware of an AEL router — the event-routing part of a Module Hub, or a standalone AEL Hub. What AEL is and the behavior AES requires — a small typed event frame carrying only an event identifier and an integrity check, receive and source bindings with per-state enables, the `ARMED` boundary, no retry, logged and counted events, and store-and-forward routing with a published timing contract — are defined by AES ([Interfaces and Versioning §4](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#4-auriora-event-link), `AES-AEL-001` to `AES-AEL-005`, [AEL specification](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/interfaces/ael.md)); this section states how firmware implements them. AEL is Module-to-Module and is unrelated to the Unit-to-host synchronization signal of §14.1.

**Module firmware**

- **Receive the frame in hardware; timestamp the reference instant in hardware.** The frame is four 8N1 characters at 1 Mbit/s, delimited by idle; receive them through a serial peripheral into DMA or a hardware FIFO, and capture the frame reference instant — the mark → space edge of the first start bit after the idle gap — by a hardware mechanism of your choice: a timer input-capture channel or programmable-I/O block watching the same line, or a UART feature that latches the start edge. Never poll, and never read a clock when the bytes are processed. Where the Module has a sample clock (audio frames, ADC samples), convert the capture to that time base and record the event as a sample or frame index as well as in local time; the sample index is what later correlation uses.
- **Validate before anything else — the gap is part of the frame.** A candidate frame is the characters between two qualifying idle gaps (at least two character times each). It is valid only if exactly four characters arrived, none carried a framing error, the CRC-16 (poly `0x1021`, init `0xFFFF`, no reflection, no xorout; `check` `0x29B1`) over the first two bytes matches the last two, and the identifier is not `0x0000`. Anything else — three or five bytes, a break, a CRC mismatch, `0x0000` — is an *invalid frame*: count it, log it with the reason, wait for the next qualifying gap, and do nothing. Use a UART idle-line or receive-timeout feature to detect the gap where the peripheral offers one; compute the CRC in hardware only if the controller has a compatible unit — over two bytes a table or bitwise software CRC is entirely acceptable, and the *result* is what the standard fixes. Consequence to plan for: the frame is confirmed only by the gap that follows it, so the earliest action is about one frame plus one gap (≈ 60 µs at 1 Mbit/s) after the reference instant — the configured delay still counts from the captured reference instant. Line activity, a break or a half-frame is never an event.
- **The binding table is the whole decision.** Hold receive bindings as a table `event_id → (action, enabled-state mask)` in validated runtime configuration (Section 13): stored with version and integrity check, checked against the actions this Module actually supports and its declared binding limit, rejected with a defined error otherwise, readable over MCI, and reported in the Module's status so the host records it with the experiment. `0x0000` is never a valid key. A first implementation with one bound action is complete.
- **Bounded lookup, deterministic action.** The lookup — identifier known? bound? enabled in the current state? — is a table operation and MAY run at DMA-completion or ISR level where its worst case is fixed and small (Section 8); the action it selects is then scheduled on a hardware timer or against the sample clock from the *captured reference instant*, not from when the ISR ran or the task woke. Whichever split is chosen, measure it: frame-to-action latency and jitter are part of the Module's published timing contract.
- **Every state × event pair is explicit.** Model the lifecycle as named states (`IDLE → CONFIGURED → ARMED → RUNNING → COMPLETE`, Section 7) and put AEL reception into the transition table of every state. Nothing is enabled before `ARMED`. In `ARMED` and `RUNNING` each binding's enable mask decides; `RUNNING` does not globally ignore events — a recording Module accepts markers while it records, a stimulus Module accepts a phase change while it plays. An event that is unknown, unbound or disabled in the current state is an explicit no-op that logs an ignored event with a reason (`UNKNOWN`, `UNBOUND`, `NOT_ARMED`, `DISABLED_IN_STATE`) — never a silent drop and never an implicit restart, stop, queue or advance. Restart, queue, advance and phase-change exist only as explicitly bound actions. `COMPLETE → IDLE` (one-shot) versus `COMPLETE → ARMED` (repeat-armed) is a configuration value, readable over MCI.
- **AEL OUT marks the real event.** Hold source bindings as `internal event → event_id` in the same validated configuration. Emit the four characters contiguously (DMA, never a byte-at-a-time loop that can open a gap inside the frame) and hold at least two character times of mark before the next frame. Emit the frame from the internal event it is bound to — the block in which the first stimulus sample actually leaves the converter, the detector's threshold crossing, the protocol phase end — through a hardware-driven serial output whose start is timed against a timer or the sample clock, compensating known fixed pipeline latency where it has been characterized. Do not emit on command receipt, and do not forward a received event unless that forwarding is itself a configured source binding. Never emit `0x0000`.
- **No frame from power, reset or firmware start.** Keep the transmitter's driver disabled, or the line in idle, until the peripheral is configured and the state machine is in a defined state; re-check on every reset path, including watchdog and fault resets. A Module that emits a frame while booting is a Module that triggers its neighbours by rebooting.
- **Never abuse the frame.** The identifier is the only semantic content. Firmware MUST NOT encode information in frame timing, spacing or repetition, MUST NOT interpret any such pattern, and MUST NOT acknowledge or retransmit an event frame. Anything that needs a type beyond an identifier, a parameter or a payload goes over MCI.
- **Log and count.** Record every received frame (identifier, acted or ignored, reason), every transmitted frame and every invalid frame (Section 12) with local timestamp and sample index, current state, related run or Session identifier, and the local counters — received valid, received invalid, ignored, transmitted. Counters are diagnostics readable over MCI, reset only explicitly, and local: never present them as synchronized with another device.
- **Host loss is a scenario, not a fault.** If the Module declares autonomous continuation and its deployment policy says *continue*, an armed or running Session continues from committed local state and active bindings when the host disappears; if it does not declare it, the documented behavior on host loss applies. Either way, a reconnection changes nothing on its own (§14.3), and a reset returns the Module to `IDLE` — it never rejoins an active experiment as `ARMED` because it remembers having been armed. The only way execution is re-entered after a reset is the validated recovery procedure of §14.4, acting on a deployment policy the host explicitly wrote beforehand.
- **Test it.** Host-test the state machine against every state × bound identifier, plus unknown, unbound, disabled-in-state, `0x0000` and corrupt frames, plus acceptance while `RUNNING` where configured and repeat-armed cycling (Section 18); on hardware, measure source-to-frame and frame-to-action latency and jitter over a run of events under representative load and record the figures in the project documentation — they are part of the Module's timing contract and the host validates deployments against them.

**Router firmware**

- **The router is a bounded real-time subsystem.** Give the event path its own priority level, core or programmable-I/O engines, and design so that nothing the management side does — MCL traffic, host transport, Asset transfers, logging — can delay a forwarding decision beyond the documented bound. Prove it by measurement with management traffic running (Section 8 timing rules apply). An RTOS is not required; whatever scheduling is used must let the bound be demonstrated, not asserted.
- **One hardware receiver and transmitter per port.** Every AEL IN and AEL OUT, including Hub-to-Hub links, has its own peripheral and DMA path with hardware timestamping of the reference instant; the router records an *input SOF timestamp* and an *output SOF timestamp* per forwarded frame, because its latency contract is defined between them. Multiplexing ports through one peripheral in software is not a router.
- **Store-and-forward, in that order.** Receive the complete frame and its following gap → validate UART framing, CRC and identifier → look up `(ingress port, event_id)` in the active route table → enqueue on each selected output → launch the outputs together. Nothing is transmitted before validation succeeds; a corrupt ingress frame produces zero egress frames and one invalid count on the ingress port. An identifier with no route is dropped and counted as unrouted — a fact, not an error.
- **Launch fan-out together.** Where one decision selects several outputs, arm their transmissions from one timer event or equivalent hardware mechanism rather than starting them from a loop, so that the measured channel-to-channel skew is a hardware figure and not a loop-iteration figure.
- **The route table knows numbers, not meaning.** Hold it as `(port, event_id) → output set` and nothing else. It has no conditionals, no state, no notion of what an identifier means or which Module is on a port; that knowledge is the host's. Firmware MUST NOT provide any mechanism by which experiment logic could be loaded into the router.
- **Stage, validate, commit — and freeze.** Route tables arrive as whole objects: stage into a separate buffer, validate (ports exist, capacity fits, table is consistent), compute the content identity, and commit by an atomic switch between frames — never by editing the active table in place. Refuse a commit while a dependent deployment is `ARMED` or `RUNNING` unless it arrives through the defined abort or recovery operation, and make that refusal, and any abort, observable. On power loss or reset the active table is gone; the router comes up with no routes and waits for the host to verify and re-commit — it never reinstates a table from non-volatile memory on its own.
- **Deterministic queues.** Fixed capacity per output (or a documented shared scheme), a documented ordering rule (arrival order at the reference instant is the natural one), frames never interleaved or truncated on an output, the two-character inter-frame gap always honored, and a documented overflow rule — which frame is dropped when the queue is full. Every drop is counted per output and reported as a fault. No retry, no acknowledgment, no back-pressure to the source.
- **Expose everything the host needs.** Per port: received, forwarded, unrouted, invalid and overflowed counts and link state; globally: active and staged table identities, supported AEL version, port topology, route and queue capacity, and the measured timing figures. Counters are local and reset only explicitly.
- **Test it under load.** Saturation to the documented rate with the overflow counts equal to the frames lost; corrupt ingress frames producing zero egress; unrouted identifiers counted and not forwarded; a commit attempt during an armed deployment refused; power and reset cycling producing zero frames on every output; and the timing figures re-measured after any change to management firmware, because that is where the regressions come from.

---

### 14.3 Module Control Interface (MCI)

These rules apply to firmware in a Module that exposes a control surface to a host. What MCI is and what AES requires of it — one transport-independent contract, persistent identity, declared capabilities, an observable lifecycle with an ARM boundary, staged object transfer and stored object retrieval — is defined by AES ([Interfaces and Versioning §5](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#5-module-control-interface), `AES-MCI-001` to `AES-MCI-005` and `AES-MCI-008`; autonomous operation, `AES-MCI-006`/`-007`, is §14.4); this section states how firmware implements them. MCI is the Module-to-host control path and is unrelated to the Managed Unit API of §14.1, which is how a Module talks *down* to its Units.
...
- **Capabilities are a table, not an assumption.** Keep the declared capability set in one place — including the AEL properties a host plans against: AEL IN/OUT, AEL version, binding limits, timing envelope — and generate the dispatch checks from it, so that what the Module *says* it supports and what it *actually* accepts cannot drift. An operation outside the declared set MUST return a defined "unsupported" error. Silent success is the failure the host cannot see — accepting a configuration write for a feature the build does not have is worse than refusing it.
- **The lifecycle is an explicit state machine.** Model `IDLE → CONFIGURED → ARMED → RUNNING → COMPLETE` with named states (Section 7), and put every MCI operation into the transition table. A refused transition MUST report *why* — invalid configuration, missing asset, unverified session, blocking fault — not merely that it failed. This is the same state machine an AEL event enters through (§14.2); arming is where the two interfaces meet.
...
- **Never let control be the trigger.** Where several Modules must act together, firmware MUST NOT provide a control operation that is documented or implied as the way to start them simultaneously. The deterministic event belongs to AEL (§14.2). A single-Module start operation is legitimate and should say plainly in its documentation that it carries no inter-Module timing guarantee.
...
- **Configuration readback is part of the contract.** The active configuration, the AEL receive and source bindings with their per-state enables, delays, post-completion mode, selected session and firmware version MUST all be readable (Section 13), because an experiment that cannot be read back cannot be reproduced from its records.
- **A completed object never changes.** Once a recording, log or committed object is complete, its bytes, size and integrity value are frozen ([AES-MCI-008](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#aes-mci-008-stored-object-retrieval)): close it, compute and store the whole-object digest with its metadata, and serve it from then on as an immutable object. Never append to, compact or rewrite a completed object in place; a recording that must be split is two objects. Deletion, where the product offers it, is a separately declared operation and is refused while the run that produced the object is `ARMED` or `RUNNING`.
- **Serve reads statelessly, in bounded chunks.** A read is (object identity, offset, length) → (the same three plus data); answer it from storage and forget it. No open handle the host must close, no sequence the host must follow, no timer that expires when the host disappears — a Module that was left alone for weeks is read by a host that may vanish again mid-object. Declare the maximum chunk length in the capability table, refuse longer requests with a defined error, and never send a chunk that was not requested. Any internal caching is invisible to the host.
- **Refuse rather than substitute.** A request for an object that no longer exists, or whose identity no longer matches its content, MUST fail with the defined error — never serve another object's bytes or a stale copy under the same identity.
- **Metadata is written when the object is, not reconstructed later.** Run and segment identity, deployment and Session identity with its hash, first and last local time and sample index, format identifier and version, firmware version and completion state go into the inventory as the recording is opened, checkpointed and closed (§14.4 persistent state); a recording that ended in a fault or on storage exhaustion is marked so. The inventory MUST be readable before any object is, and every recording belongs to exactly one run segment.
- **Reading a growing recording, if offered.** Where the product declares that an in-progress recording is readable, expose only bytes already committed to storage — a stable prefix — and report the object as in progress with its current readable length; size and digest appear on completion. A read MUST NOT observe a buffer that a later write could change.
- **Retrieval never steals from acquisition.** Serve retrieval from a lower-priority task than acquisition, AEL handling and control; bound storage reads to one chunk per request; and measure that the sample clock, the AEL frame-to-action latency and the control response time are unchanged while the host reads at full rate. Where the product cannot guarantee that in `ARMED`/`RUNNING`, refuse retrieval in those states with a defined error rather than degrade silently.
...
### 14.4 Autonomous Operation and Recovery

These rules apply to firmware in a Module that declares **autonomous continuation** — continuing a Session without a host — and in particular to one that also declares a recovery behavior after reset. What AES requires — a declared capability, a deployment policy configured only from declared behaviors, `IDLE` after every reset, recovery only on validated persistent deployment state, run segments with a recorded boundary, storage exhaustion always logged — is defined by AES ([AES-MCI-006](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#aes-mci-006-autonomous-continuation-and-deployment-policy), [AES-MCI-007](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/05-interfaces-and-versioning.md#aes-mci-007-recovery-after-reset-and-run-segment-provenance), [EDR-009](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/edr/EDR-009-autonomous-module-operation-and-recovery.md)); this section states how firmware implements it. A Module that declares none of this is unaffected, apart from the reset rule it already follows.

- **Boot to `IDLE`, then decide.** Every reset path — power-on, brownout, watchdog, software, external — enters the same startup sequence: outputs safe (Section 2.3, [AES-CTRL-002](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/03-architecture.md#aes-ctrl-002-deterministic-startup)), reset cause read and logged (Section 6), lifecycle `IDLE`. Only then does the recovery procedure run: load and validate the persistent deployment state; if its policy authorizes recovery, re-run the *same* prerequisite and arming checks the `arm` operation performs — configuration and Session present with matching integrity values, required Units rediscovered and ready, storage mounted and consistent — and transition `IDLE → CONFIGURED → ARMED → RUNNING` through the ordinary state machine (Section 7) with `RECOVERY` as the transition reason. There is no shortcut into `RUNNING` and no code path that restores a lifecycle state from memory. If any check fails, stay in `IDLE`, record `RECOVERY_REFUSED` with the reason, and leave the persistent state and the data exactly as they were for the host to read.
- **Persistent deployment state is a versioned, integrity-checked record, written atomically.** Treat it as runtime configuration under Section 13 — schema version, CRC or digest, validated on load, corrupt-or-newer means no recovery — and write it so that a power loss mid-write leaves the previous copy intact: two slots with a sequence number, or a journaled write-then-switch, following the same discipline as the firmware update of Section 16. Update it at every lifecycle transition and segment boundary, and checkpoint it at a bounded cadence during a run with the current local time and sample index, so the ended segment's last known point is known after a reset. Never write it per sample: the checkpoint cadence is a trade between the gap you can describe and the endurance of the memory, and it is documented.
- **Bound the recovery loop.** Increment the persisted consecutive-recovery counter *before* attempting recovery and clear it only when the run reaches a defined stable point — a completed checkpoint interval is a reasonable one. A counter at its declared bound means the Module stays `IDLE` with `RECOVERY_REFUSED (RECOVERY_BOUND_REACHED)` rather than minting a new segment every few minutes for a fault that returns on every boot.
- **Every recovery opens a segment; nothing is stitched.** Allocate a new run segment identifier (under *resume*) or a new run identifier with segment 0 (under *restart*), restart the sample index from zero for the new segment, and record the boundary: reset cause, last persisted local time and sample index, first new local time and sample index, the time-reference status (RTC present and battery-backed, free-running, externally disciplined, unknown) and the authorizing policy. The sample index MUST NOT continue across the boundary as if no samples were missing; the gap is described, not hidden.
- **Host loss is detected, never felt.** Define host loss as the Module or its transport binding defines it — a supervision interval without MCI activity, or a loss the transport reports — and implement it as one input to the state machine. Under *continue*, no link event of any kind may reach the Session logic: not a USB suspend, not a Hub port dropping, not re-enumeration. Under *stop*, host loss runs the Module's ordinary abort path and logs `HOST_LOST` as its reason; it is not a fatal error and it does not reset anything.
- **Storage exhaustion is a policy branch, not an exception handler.** Watch free space ahead of the write that would fail. On exhaustion execute the configured behavior and nothing else: under *stop*, end recording, keep every stored sample, log `STORAGE_EXHAUSTED` and complete the run with that reason; under *overwrite oldest*, discard whole oldest units of data, keep a persistent record of the overwritten extent, and log the first overwrite. Overwrite is never the fallback when no behavior was configured, and the free-space figure the host reads is the one the Module actually uses.
- **Fault recovery is a table with a bound per row.** Hold the Module's fault classes as data: recoverable (retry a Unit initialization, reopen a storage file), degraded (continue with an optional Unit absent, flagged), fatal for the run (a required Unit gone, storage unrecoverable). Each recoverable class carries a retry bound; every attempt, success, degradation and give-up is logged with the class and the count, and a fault that ends the run persists its reason before the transition. The table is documented with the Module, because the same fault is recoverable for a logger and fatal for a stimulus Module.
- **Complete with nobody watching.** Reaching `COMPLETE` — by the Session's end, by *stop* on storage exhaustion, by a fatal fault — persists the completion or stop status, its reason, the local time and sample index, and the final segment before the state is reported anywhere. The event log, the data and the persistent state survive until a host reads them; a Module that clears any of them on a timer, on boot or on reconnection has thrown away the experiment.
- **Everything the host reads is the truth, including the gaps.** Expose over MCI (§14.3 readback): the active deployment policy, whether recovery occurred and how often, current run and segment identity, the last reset cause, completion/stop/refusal reason, storage free and total, and the overwritten extent if any. These come from the persistent state and the log, never from what the firmware would like to have happened.
- **Test the reset, not just the run.** Power-cycle the Module in every lifecycle state and confirm it boots to `IDLE` and recovers only where the policy allows; corrupt the persistent state and confirm no recovery and a logged refusal; remove a required Unit before boot and confirm `RECOVERY_REFUSED (MISSING_PREREQUISITE)`; force a recurring fault and confirm the bound; fill storage under both behaviors; drop the host under both host-loss behaviors, including through a USB suspend; and read every segment boundary back and check that no sample index continues across it. Hardware testing of release candidates already requires power cycling and watchdog recovery (Section 18) — this is what those cycles are checked against.

---

### 14.5 Module Hub Host-Facing Interface and MCI Forwarding

These rules apply to firmware in a Module Hub — the `MCL` concentrator and the host-facing interface through which the host reaches the Hub and every Module behind it ([AES-HUB-003](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/03-architecture.md#aes-hub-003-host-facing-interface-and-traffic-separation)). The AEL router of §14.2 shares the controller and none of these rules.

- **Two kinds of endpoint on one connection.** The host-facing interface is a transport adapter of the direct local MCI binding, with the addressing that binding defines: the Hub is an addressable endpoint for its own management, and every Module Port is a forwarding path. The Hub's own management operations follow the same discipline as a Module's (§14.3) — one semantic implementation, adapters that only frame.
- **Forward, never interpret.** A request for a Module is relayed to that Module Port and its response relayed back. The Hub does not parse, cache, reorder, merge, answer on behalf of, or time out for the Module beyond the binding's own per-port timeout; stored objects are bytes it never looks inside.
- **Buffer only what was asked for.** Hold at most the responses to requests the host has outstanding, per port; never prefetch, never accept unsolicited data from a Module. A host that reads slowly therefore stalls its own requests and nothing else, and the binding's flow control makes a Module that cannot be drained visible rather than silently overrun.
- **Interleave at chunk granularity.** Schedule the host-facing link and each `MCL` link so that a control request to any port is transmitted within one bulk chunk plus framing overhead of its arrival; a bulk chunk is never split and never queued several deep ahead of control on the same link. Measure it with one port downloading at full rate and control traffic on another.
- **Per-port independence, as required.** A stuck, unpowered or disconnected Module on one port MUST NOT stall forwarding on another ([AES-HUB-002](https://github.com/auriora-org/auriora-engineering-standard/blob/main/docs/03-architecture.md#aes-hub-002-port-independence-and-scale-interoperability)): independent link state, timeout and buffers per port, and a fault reported as that port's fault.
- **The router is untouched by any of this.** The AEL path of §14.2 runs at its own priority with its own peripherals; the forwarding rate has no effect on the published forwarding latency, and the acceptance test drives the host-facing link and every `MCL` port at maximum bulk rate while measuring the event path.
- **USB is an adapter, not an architecture.** Where the host-facing interface is USB, the Hub is the device; the USB class and descriptors are the binding specification's and the product's choice, isolated in the adapter so that a second host transport later is a second adapter, not a second Hub.
- **Test disconnect on both sides.** Pull the host cable mid-download and confirm every `MCL` port is idle within the binding's timeout and no Module was left mid-request; pull a Module mid-download and confirm the host receives that port's defined error while every other port continues.
...
*AURIORA Firmware Style Guide 0.5.0 — complements the AURIORA Engineering Standard. Licensed under CC BY-SA 4.0.*
